...
gint y_axis_markers_size (YAxisMarkers *gal)
{
...
static void
populate_dates_weekly (YAxisMarkers *gam,
		       GDate *min, GDate *max)
{
  gchar buf[32];
  GDate dt = *min;
...
static void
populate_dates_monthly (YAxisMarkers *gam,
			GDate *min, GDate *max)
{
  gchar buf[32];
  GDate dt, dt2;
...
static void
populate_dates_quarterly (YAxisMarkers *gam,
			  GDate *min, GDate *max)
{
  ...
  while (g_date_lteq (&dt, max)) {
    ...
    g_snprintf (buf, 32, "%c-%02d", *monthname, g_date_get_year (&dt) % 100);
...
static void
populate_dates_yearly (YAxisMarkers *gam,
		       GDate *min, GDate *max)
{
  gchar buf[32];
  GDate dt, dt2;
...
void
y_axis_markers_populate_dates (YAxisMarkers *gam,
				   GDate *min, GDate *max)
{
  gint jspan;
				   ...
				   double *max_w, double *max_h)
{
  ...
  if (max_w) {
    *max_w = 0;
	...
	if (max_w) {
	  double w = gnome_font_get_width_utf8 (f, y_tick_label (tick));
	  *max_w = MAX (*max_w, w);
	}
      }
    }
