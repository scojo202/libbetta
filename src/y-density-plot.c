...
static
GtkSizeRequestMode get_request_mode (GtkWidget *w)
{
...
static void
get_preferred_height_for_width (GtkWidget *w, gint for_width, gint *minimum, gint *natural) {
  YDensityPlot *widget = Y_DENSITY_PLOT(w);
  ...
  *minimum = (*natural >= 50 ? 50 : *natural);
  ...
  g_debug("density plot requesting height %d for width %d",*natural,for_width);
}

static void
get_preferred_width_for_height (GtkWidget *w, gint for_height, gint *minimum, gint *natural) {
  YDensityPlot *widget = Y_DENSITY_PLOT(w);
  if(widget->aspect_ratio>0) {
  ...
  *minimum = (*natural >= 50 ? 50 : *natural);
  ...
  g_debug("density plot requesting width %d for height %d, minimum %d",*natural,for_height,*minimum);
}
...
y_density_plot_update_surface (YDensityPlot *widget)
{
  ...
  g_debug("update_surface: %d -> %d, %d -> %d",size.rows, height, size.columns, width);
...
static
unsigned char b_red (float scaled) {
  if(scaled<=0.5f) return 0;
  if(scaled<0.75f) return 255*(scaled-0.5f)*4;
  if(scaled<=1.0f) return 255;
  return 0;
}

static
unsigned char b_blue (float scaled) {
  if(scaled>=0.5f) return 0;
  if(scaled>0.25f) return 255*(0.25f-scaled)*4;
  if(scaled>=0.0f) return 255;
  return 0;
}

static
unsigned char b_green (float scaled) {
  if(scaled<=0.75f && scaled>=0.25f) return 0;
  if(scaled<0.25f) return 255*(0.25f-scaled)*4;
  if(scaled>0.75f) return 255*(scaled-0.75f)*4;
  return 0;
}

static
void on_data_changed(YData *dat, gpointer   user_data) {
  ...
  for(i=0;i<nrow;i++) {
   for(j=0;j<ncol;j++) {
    if(__builtin_isnan(data[i*ncol+j])) { /* math.h isnan() is really slow */
      ...
      if(ss>=0 && ss<256) {
        pixels[n_channels*j+(nrow-1-i)*rowstride]=lut[4*ss+2];
        pixels[n_channels*j+(nrow-1-i)*rowstride+1]=lut[4*ss+1];
        pixels[n_channels*j+(nrow-1-i)*rowstride+2]=lut[4*ss];
      }
      else {
        //cbuffer[i]=0;
      }
...
y_density_plot_draw (GtkWidget *w, cairo_t *cr)
{
  ...
  YViewInterval * vix = y_element_view_cartesian_get_view_interval (Y_ELEMENT_VIEW_CARTESIAN(widget), X_AXIS);
  if(vix != NULL) {
  ...
  if(widget->scaled_pixbuf != NULL)
    g_object_get(widget->scaled_pixbuf,"height",&pheight,"width",&pwidth,NULL);
  
  if(pheight != height || pwidth != width) {
    if(widget->scaled_pixbuf != NULL)
      g_object_unref(widget->scaled_pixbuf);
    widget->scaled_pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB,FALSE,8,width,height);
  }
  ...
  gdk_pixbuf_scale(widget->pixbuf,widget->scaled_pixbuf,
		   ...
		   GDK_INTERP_TILES);
#if CREATE_SURF
  cairo_surface_t *surf = gdk_cairo_surface_create_from_pixbuf(widget->scaled_pixbuf,0,gtk_widget_get_window(GTK_WIDGET(widget)));
...
static gboolean
y_density_plot_configure_event (GtkWidget *w,
...
{
  y_element_view_cartesian_set_preferred_view ((YElementViewCartesian *)gev, X_AXIS);
  y_element_view_cartesian_set_preferred_view ((YElementViewCartesian *)gev, Y_AXIS);
                        ...
                        GParamSpec   *pspec)
{
    ...
    case DENSITY_PLOT_DATA: {
      ...
      if(self->tdata) {
        ...
        self->tdata_changed_id = g_signal_connect_after(self->tdata,"changed",G_CALLBACK(on_data_changed),self);
      }
      break;
    }
...
y_density_plot_init (YDensityPlot *plot)
{
  ...
  gtk_widget_add_events(GTK_WIDGET(plot),GDK_SCROLL_MASK | GDK_BUTTON_PRESS_MASK);

  g_object_set(plot,"expand",FALSE,"valign",GTK_ALIGN_START,"halign",GTK_ALIGN_START,NULL);
}
